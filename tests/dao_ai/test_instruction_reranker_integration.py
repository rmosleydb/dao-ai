...
class TestStructuredOutputWithDatabricks:
    ...
    def test_with_structured_output_json_schema_method(
        self, llm: ChatDatabricks
    ) -> None:
        """Test with_structured_output using json_schema method."""
        structured_llm = llm.with_structured_output(RankingResult, method="json_schema")
        ...
        print("\n=== JSON Schema Method Results ===")
        print(f"Number of rankings: {len(result.rankings)}")
        for r in result.rankings:
            print(f"  [{r.index}] score={r.score:.2f} - {r.reason[:80]}...")

    def test_with_bind_response_format(self, llm: ChatDatabricks) -> None:
        """Test using llm.bind with Databricks-compatible response_format."""
        import json

        from dao_ai.utils import get_databricks_response_format

        response_format = get_databricks_response_format(RankingResult)
        bound_llm = llm.bind(response_format=response_format)

        response = bound_llm.invoke(TEST_PROMPT)
...
@pytest.mark.integration
@pytest.mark.skipif(not HAS_DATABRICKS_CREDS, reason=SKIP_MSG)
class TestStructuredOutputMethods:
    """Compare different structured output methods with Databricks."""
    ...
    def test_compare_methods(self, llm: ChatDatabricks) -> None:
        """Compare tool calling vs json_schema vs bind methods."""
        import json
        ...
        # Method 1: Default (tool calling)
        try:
            structured_llm = llm.with_structured_output(RankingResult)
            result = structured_llm.invoke(TEST_PROMPT)
            results["tool_calling"] = result
            print("\n[PASS] Tool calling method succeeded")
        except Exception as e:
            errors["tool_calling"] = str(e)
            print(f"\n[FAIL] Tool calling method: {e}")
        ...
        try:
            ...
            result = structured_llm.invoke(TEST_PROMPT)
            results["json_schema"] = result
            print("[PASS] JSON schema method succeeded")
        except Exception as e:
            errors["json_schema"] = str(e)
            print(f"[FAIL] JSON schema method: {e}")

        # Method 3: Bind with Databricks response format
        try:
            response_format = get_databricks_response_format(RankingResult)
            bound_llm = llm.bind(response_format=response_format)
            response = bound_llm.invoke(TEST_PROMPT)
            ...
            result = RankingResult.model_validate(result_dict)
            results["bind_response_format"] = result
            print("[PASS] Bind response format method succeeded")
        except Exception as e:
            errors["bind_response_format"] = str(e)
            print(f"[FAIL] Bind response format method: {e}")

        # At least one method should work
        assert len(results) > 0, f"All methods failed: {errors}"

        # Print summary
        print("\n=== Summary ===")
        print(f"Successful methods: {list(results.keys())}")
        print(f"Failed methods: {list(errors.keys())}")
